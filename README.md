...
各功能包功能如下：
- mybot_description  机器人描述文件，包含方针相关配置
- mybot_navigation2  机器人导航配置文件
- mybot_application  机器人导航应用 Python 代码
...
2. 安装仿真相关功能包
```shell
sudo apt install ros_$ROS_DISTRO-robot-state-publisher ros-$ROS_DISTRO-joint-state-publisher ros-$ROS_DISTRO-gazebo-ros-pkgs ros-$ROS_DISTRO-ros2-controllers ros-$ROS_DISTRO-xacro
```
...
```shell
source install/setup.bash
ros2 autopatrol_robot autopatrol.launch.py
```
